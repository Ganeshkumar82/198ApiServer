...
router.post('/devicenotify', async function(req,res,next){
  try{
    res.json(await event.DeviceNotify(req.body));
  }catch(er){
    console.error(`Error while fetching the Whatsapp group name`, er.message);
    next(err);
  }
});

router.post('/acknowledged', async function(req,res,next){
  try{
    res.json(await event.Acknowledge(req.query.page,req.body));
  }catch(er){
    console.error(`Error while fetching the Whatsapp group name ${er.message}`);
    next(er);
  }
...
router.post('/checkcameraactive', async function(req,res,next){
  try{
    res.json(await event.CheckCameraActive(req.body));
  }catch(er){
    console.error(`Error while checking the camera status -> ${er.message}`);
    next(er);
  }
});

router.post('/whatsappwall', async function(req,res,next){
  try{
    res.json(await event.WhatsappWall(req.body));
  }catch(er){
    console.error(`Error while checking the whatsapp group for group id -> ${er.message}`);
    next(er);
  }
});

router.post('/geteventcount', async function(req,res,next){
 try{
   res.json(await event.GetEventCount(req.body));
 }catch(er){
   console.error(`Error while fetching the event count for the user -> ${er.message}`);
   next(er);
 }
});

router.post('/getbreakduration' , async function(req,res,next){
  try{
    res.json(await event.GetBreakDuration(req.body));
  }catch(er){
    console.error(`Error while Fetching the break duration -> ${er.message}`);
    next(er);
  }
});
...
router.post('/addeventforai' , async function(req,res,next){
  try{
    res.json(await event.AddEventforAI(req.body));
  }catch(er){
...
router.post('/fetchwhatsappbycam', async function(req,res,next){
  try{
    res.json(await event.FetchWhatsappbyCam(req.body));
  }catch(er){
    console.error(`Error while fetching the Whatsapp group name by camera id -> ${er.message}`);
    next(er);
  }
});

router.post('/userdailyreport', async function(req,res,next){
  try{
    res.json(await event.userDailyReport(req.body));
  }catch(er){
    console.error(`Error while Fetching the Users daily reportt -> ${er.message}`);
    next(er);
  }
});
