    ...
    //3. Getting Site dept list
     ...
     router.post('/getcontactlist',async function(req,res,next){
      try{
        ...
        //// console(`Error while fetching customer contact list`,err.message);
        next(err);
      }
     })
//Getting the customer billing list
 ...
 router.post('/getinvoicelist',async function(req,res,next){
  try{
    ...
    //// console('Error while fetching customer invoice list',err.message);
    next(err);
  }
 });
 //GETTING THE CUSTOMER PAYMENT LIST
  router.post('/getpaymentlist',async function(req,res,next){
    try{
      ...
      //// console('Error while fetching customer payment list',err.message);
      next(err);
    }
  });
  ...
  router.post('/getpaymentbank',async function(req,res,next){
    try{
      ...
      //// console("Error while fetching customer payment bank list",err.message);
      next(err);
    }
  });
...
router.post('/getdeviceinfo',async function(req,res,next){
  try{
    res.json(await customer.getDeviceInfo(req.body));
  }catch(err){
    console.error('Error while Fetching the device info',err.message);
    next(err);
  }
});
...
router.post('/enquiry', async function(req,res,next){
  try{
    res.json(await customer.EnquiryCustomer(req.body));
  }catch(er){
    console.error(`Error while adding the enquiry customer -> ${er.message}`);
    next(er);
  }
});

router.post('/livechat', async function(req,res,next){
  try{
    res.json(await customer.LiveChat(req.body));
  }catch(er){
    console.error(`Error while fetcing the chatlist -> ${er.message}`);
    next(er);
  }
});
