...
router.post('/adddetails', async function(req,res,next){
  try{
    ...
    console.error(`Error while adding the details of the secure shutter -> ${er.message}`);
    next(er);
  }
});
...
router.post('/getdetails', async function(req,res,next){
  try{
    ...
    console.error(`Error while Fetching the details of the secure shutter -> ${er.messsage}`);
    next(er);
  }
});
...
router.post('/updatedetails', async function(req,res,next){
    try{
      ...
      console.error(`Error while Updating the details of the secure shutter -> ${er.message}`);
      next(er);
    }
});
