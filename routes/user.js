...
router.post('/userbreak',async function(req,res,next){
   try{
     res.json(await user.UserBreak(req.body));
   }catch(er){
     console.error(`Error updating the user break completion -> ${er.message}`);
     next(er);
   }
});
