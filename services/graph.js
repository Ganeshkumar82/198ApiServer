...
async function Sitesecondgraph(graph){
    try{
       const sql = await db.query(`SELECT bm.branch_id, bm.branch_name, CASE WHEN bm.sitecontroller_status = 1 THEN DATE_FORMAT(bm.site_uptime, '%Y-%m-%d %H:%i:%s')
...
async function Sitethirdgraph(graph) {
  try {
    ...
    const hourlyResults = motionDetectionCounts.map((motionEvent, index) => ({
      ...
      device_event_count: deviceEventsCounts[index] ? deviceEventsCounts[index].device_event_count : 0,
      videoloss_event_count: videolossEventCounts[index] ? videolossEventCounts[index].videoloss_event_count : 0,
    }));
...
async function DatabaseFirst(graph){
  try{
     const runstatus = await db.query(`select database_id from databasemaster where runningstatus = 1 and deleted_flag = 0 and status = 1;`);
     const stopstatus = await db.query(`select database_id from databasemaster where runningstatus = 0 and deleted_flag = 0 and status = 1;`);
     const totalsite = await db.query(`select database_id from databasemaster where status =1 and deleted_flag =0 `);
     ...
     return ({code:true,message:"DB percentage FEtched successfully",runningPercentage:Math.round(runningPercentage),stoppedPercentage:Math.round(stoppedPercentage)})
  }catch(er){
...
async function AppServerStatus(graph){
  try{
    const runstatus = await db.query(`select applicationserver_id from applicationservermaster where runningstatus = 1 and status = 1 and Deleted_flag = 0`);
    const stopstatus = await db.query(`select applicationserver_id from applicationservermaster where runningstatus =0 and status = 1 and deleted_flag =0`);
    const totalcount = await db.query(`select applicationserver_id from applicationservermaster where status=1 and deleted_flag =0`);
    ...
    return ({code:true,message:'Application server percentage Fetched Successfully',runningPercentage:Math.round(runningPercentage),stoppedPercentage:Math.round(stoppedPercentage)});
  }catch(er){
    return ({code:false,message:'Internal error. Please contact Administration',error:er});
  }
}
...
async function WebserverStatus(graph){
  try{
     const runstatus = await db.query(`select webserver_id from webservermaster where runningstatus = 1 and deleted_flag = 0 and status =1`);
     const stopstatus = await db.query(`select webserver_id from webservermaster where runningstatus = 0 and deleted_flag = 0 and status =1`);
     const totalcount = await db.query(`select webserver_id from webservermaster where status = 1 and deleted_flag =0 `);
     const runningCount = runstatus.length;
     const stoppedCount = stopstatus.length;
     const totalCount = totalcount.length;
     ...
     return ({code: true,message:"Webserver percentage Fetched Successfully.",runningPercentage:Math.round(runningPercentage),stoppedPercentage:Math.round(stoppedPercentage)});
  }catch(er){
     return ({code: false,message:"Internal error. Please contact Administration",error:er});
  }
}
...
async function AIserverStatus(graph){
  try{
     const runstatus = await db.query(`select machine_id from aisysmaster where runningstatus = 1 and status =1 and deleted_flag = 0`);
     const stopstatus = await db.query(`select machine_id from aisysmaster where runningstatus = 0 and status =1 and deleted_flag =0`);
     const totalcount = await db.query(`select machine_id from aisysmaster where status =1 and deleted_flag =0`);
     const runningCount = runstatus.length;
     const stoppedCount = stopstatus.length;
     const totalCount = totalcount.length;
     const runningPercentage = (runningCount / totalCount) * 100;
     ...
     return ({code: true, message:'AI server percentage Fetched Successfully',runningPercentage:Math.round(runningPercentage),stoppedPercentage:Math.round(stoppedPercentage)});
  }catch(er){
     return ({code: false, message:"Internal error. Please contact Administration",error:er});
  }
}
...
async function ComServerStatus(graph){
   try{
    const runstatus = await db.query(`select comserver_id from communicationserver where runningstatus = 1 and status = 1 and deleted_flag = 0`);
    const stopstatus = await db.query(`select comserver_id from communicationserver where runningstatus = 0 and status = 1 and deleted_flag = 0`);
    const totalcount = await db.query(`select comserver_id from communicationserver where status = 1 and deleted_flag = 0`);
    const runningCount = runstatus.length;
    const stoppedCount = stopstatus.length;
    const totalCount = totalcount.length;
    ...
    const runningPercentage = (runningCount / totalCount) * 100; 
    const stoppedPercentage = (stoppedCount / totalCount) * 100;
    return ({code: true, message:'Communication server percentage Fetched Successfully',runningPercentage:Math.round(runningPercentage),stoppedPercentage:Math.round(stoppedPercentage)});
  }catch(er){
     return ({code: false, message:"Internal error. Please contact Administration",error:er});
  }
}
...
async function LoadBalancerServer(graph){
   try{
     const runstatus = await db.query(`select loadbalancer_id from loadbalancermaster where running_status = 1 and status = 1 and deleted_flag = 0`);
     const stopstatus = await db.query(`select loadbalancer_id from loadbalancermaster where running_status = 0 and status = 1 and deleted_flag = 0`);
     const totalcount = await db.query(`select loadbalancer_id from loadbalancermaster where status = 1 and deleted_flag = 0`);
     const runningCount = runstatus.length;
     const stoppedCount = stopstatus.length;
     const totalCount = totalcount.length;
     ...
     const runningPercentage  = (runningCount / totalCount) * 100;
     const stoppedPercentage = (stoppedCount / totalCount) * 100;
     return ({code: true, message:"Load Balancer server running percentage Fetched Successfully",runningPercentage:Math.round(runningPercentage),stoppedPercentage:Math.round(stoppedPercentage)});
   }catch(er){
     return ({code:false, message:"Internal error. Please contact Administration",error:er});
   }
}
...
async function NotifyServerStatus(graph){
  try{
    const runstatus = await db.query(`select notification_id from notificationmaster where server_status = 1 and status = 1 and deleted_flag = 0`);
    const stopstatus = await db.query(`select notification_id from notificationmaster where server_status = 0 and status = 1 and deleted_flag = 0`);
    const totalcount = await db.query(`select notification_id from notificationmaster where status = 1 and deleted_flag = 0`);
    const runningCount = runstatus.length;
    const stoppedCount = stopstatus.length;
    const totalCount = totalcount.length;
    ...
    const runningPercentage = (runningCount / totalCount) * 100;
    const stoppedPercentage = (stoppedCount / totalCount) * 100;
     return ({code : true, message:"Notification server running percentage status Fetched Successfully",runningPercentage:Math.round(runningPercentage),stoppedPercentage:Math.round(stoppedPercentage)});
  }catch(er){
     return ({code:false,message:"Internal error. Please contact Administration",error:er});
  } 
}
...
async function FileServerStatus(graph){
  try{
    const runstatus = await db.query(`select storagepath_id from storagepathmaster where running_status = 1 and status =1 and deleted_flag = 0 and Storage_type in(1,2)`);
    const stopstatus = await db.query(`select storagepath_id from storagepathmaster where running_status = 0 and status = 1 and deleted_flag = 0 and Storage_type in(1,2)`);
    const totalcount = await db.query(`select storagepath_id from storagepathmaster where status = 1 and deleted_flag = 0 and Storage_type IN(1,2)`);
    const runningCount = runstatus.length;
    const stoppedCount = stopstatus.length;
    const totalCount = totalcount.length;
    ...
    const runningPercentage = (runningCount / totalCount) * 100;
    const stoppedPercentage = (stoppedCount / totalCount) * 100;
      return ({code: true, message:"File server running percentage status Fetched Successfully",runningPercentage:Math.round(runningPercentage),stoppedPercentage:Math.round(stoppedPercentage)});
  }catch(er){
      return ({code:false,message:"Internal error. Please contact Administration",error:er});
  }
}
...
async function DeviceControllerStatus(graph) {
  ...
  var latestStatus;
    ...
    try{
    const latestStatusQuery = `SELECT device_id, status FROM (SELECT device_id, status,ROW_NUMBER() OVER (PARTITION BY device_id ORDER BY devicelog_id DESC) AS rn
        FROM devicerunningstatuslog WHERE device_id IN (SELECT device_id FROM devicemaster WHERE dept_id IN (SELECT dept_id FROM deptmaster WHERE branch_id IN (${graph.siteid})))) sub WHERE rn = 1;`;
    ...
    return {
      code: true,
      message: "Device controller running percentage status fetched successfully",
...
async function SiteControllerStatus(graph){
  try{
    const runstatus = await db.query(`SELECT branch_id from branchmaster where Sitecontroller_status =1 and status =1 and deleted_flag =0;`);
    const stopstatus = await db.query(`SELECT branch_id from branchmaster where Sitecontroller_status =0 and status =1 and deleted_flag =0;`);
    const totalcount = await db.query(`SELECT branch_id from branchmaster where status =1 and deleted_flag =0;`);
    const runningCount = runstatus.length;
    const stoppedCount = stopstatus.length;
    const totalCount = totalcount.length;
    ...
    const runningPercentage = (runningCount / totalCount) * 100;
    const stoppedPercentage = (stoppedCount / totalCount) * 100;
    return ({code: true, message:"Site controller running percentage status Fetched Successfully",runningPercentage:Math.round(runningPercentage),stoppedPercentage:Math.round(stoppedPercentage)})
  }catch(er){
      return ({code:false,message:"Internal error. Please contact Administration",error:er});
  }
}
  ...
  try {
    ...
    if (graph.fetchtype === 'daily') {
      sql = await db.query(`
        ...
        WHERE sc.siteaction = 0 AND DATE(sc.Row_updated_date) = ?`, [formattedDate]);
      ...
      sql = await db.query(`
        ...
        FROM sitecontrollerstatus sc 
        JOIN branchmaster bm ON sc.site_id = bm.Branch_id 
        WHERE sc.siteaction = 0 AND DATE(sc.Row_updated_date) BETWEEN ? AND ?`, [startDateFormatted, formattedDate]);
    } else if (graph.fetchtype === 'monthly') {
      startDate = new Date(currentDate);
    ...
    sql.forEach(row => {
      ...
      const dayFormatted = `${String(downtimeDate.getDate()).padStart(2, '0')}/${String(downtimeDate.getMonth() + 1).padStart(2, '0')}`;
      ...
      sites[id].downtimes[dayFormatted].push({ hour: hours, originalMin: minutes, min: dividedMinutes });
    });
...
async function DeviceOverallReport(graph) {
  try {
    if (!graph.hasOwnProperty('fetchtype')) {
      return helper.getErrorResponse(false, "Fetching type missing. Please provide the Fetch type", "DEVICE OVERALL STATUS", "");
    }
    ...
    if (graph.fetchtype === 'daily') {
      ...
      return helper.getErrorResponse(false, "Invalid fetching type. Please provide a valid fetching type (daily, weekly, monthly).", "DEVICE OVERALL STATUS");
    }
    ...
    sql.forEach(row => {
      ...
      const dayFormatted = downtimeDate ? `${String(downtimeDate.getDate()).padStart(2, '0')}/${String(downtimeDate.getMonth() + 1).padStart(2, '0')}` : null;
      ...
      if (dayFormatted) {
        devices[id].downtimes[dayFormatted] = { hour: hours, originalMin: minutes, min: dividedMinutes };
      }
    });
...
async function AIOverallDowntime(graph) {
  try {
    if (!graph.hasOwnProperty('fetchtype')) {
      return helper.getErrorResponse(false, "Fetching type missing. Please provide the Fetching type", "AI MACHINE OVERALL STATUS", "");
    }
    ...
    if (graph.fetchtype === 'daily') {
      sql = await db.query(`
        ...
        WHERE am.Action_type = 0 AND DATE(am.Row_updated_date) = ?`, [formattedDate]);
    } else if (graph.fetchtype === 'weekly') {
      startDate = new Date(currentDate);
      startDate.setDate(currentDate.getDate() - 6);
      startDateFormatted = `${startDate.getFullYear()}-${String(startDate.getMonth() + 1).padStart(2, '0')}-${String(startDate.getDate()).padStart(2, '0')}`;
      sql = await db.query(`
        ...
        FROM aimachinestatus am 
        JOIN aisysmaster ai ON ai.machine_id = am.machine_id 
        WHERE am.Action_type = 0 AND DATE(am.Row_updated_date) BETWEEN ? AND ?`, [startDateFormatted, formattedDate]);
    } else if (graph.fetchtype === 'monthly') {
      startDate = new Date(currentDate);
      startDate.setDate(currentDate.getDate() - 29);
      startDateFormatted = `${startDate.getFullYear()}-${String(startDate.getMonth() + 1).padStart(2, '0')}-${String(startDate.getDate()).padStart(2, '0')}`;
      ...
      return helper.getErrorResponse(false, "Invalid fetching type. Please provide a valid fetching type (daily, weekly, monthly).", "AI MACHINE OVERALL STATUS");
    }
    ...
    sql.forEach(row => {
      ...
      if (dayFormatted) {
        machines[id].downtimes[dayFormatted] = { hour: hours, originalMin: minutes, min: dividedMinutes };
      }
    });
...
async function NotifyDowntime(graph) {
  try {
    if (!graph.hasOwnProperty('fetchtype')) {
      return helper.getErrorResponse(false, "Fetching type missing. Please provide the Fetching type", "FETCH NOTIFICATION SERVER DOWNTIME", "");
    }
    ...
    if (graph.fetchtype === 'daily') {
      sql = await db.query(`SELECT notification_id, servername, action_datetime AS downtime 
                            FROM notificationserverstatus 
                            WHERE action_type = 0 AND DATE(Row_updated_date) = ?`, [formattedDate]);
    } else if (graph.fetchtype === 'weekly') {
      startDate = new Date(currentdate);
      startDate.setDate(currentdate.getDate() - 6);
      startDateFormatted = `${startDate.getFullYear()}-${String(startDate.getMonth() + 1).padStart(2, '0')}-${String(startDate.getDate()).padStart(2, '0')}`;
      sql = await db.query(`SELECT notification_id, servername, action_datetime AS downtime 
                            FROM notificationserverstatus 
                            WHERE action_type = 0 AND DATE(Row_updated_date) BETWEEN ? AND ?`, [startDateFormatted, formattedDate]);
    } else if (graph.fetchtype === 'monthly') {
      startDate = new Date(currentdate);
      startDate.setDate(currentdate.getDate() - 29);
      startDateFormatted = `${startDate.getFullYear()}-${String(startDate.getMonth() + 1).padStart(2, '0')}-${String(startDate.getDate()).padStart(2, '0')}`;
      ...
      return helper.getErrorResponse(false, "Invalid fetching type. Please provide a valid fetching type (daily, weekly, monthly).", "NOTIFICATION OVERALL STATUS");
    }
    ...
    sql.forEach(row => {
      ...
      if (dayFormatted) {
        notifications[id].downtimes[dayFormatted] = { hour: hours, originalMin: minutes, min: dividedMinutes };
      }
    });
    ...
    for (const [id, notificationData] of Object.entries(notifications)) {
      dateRange.forEach(date => {
        if (!notificationData.downtimes[date]) {
          notificationData.downtimes[date] = { hour: 0, originalMin: 0, min: 0 };
        }
        downtimeReport.push({
          id,
...
async function DbDowntime(graph){
  try {
    if (!graph.hasOwnProperty('fetchtype')) {
      return helper.getErrorResponse(false, "Fetching type missing. Please provide the Fetching type", "FETCH DATABASE SERVER DOWNTIME", "");
    }
    ...
    if (graph.fetchtype === 'daily') {
      sql = await db.query(`SELECT ds.dbserver_id,dm.databaseservername ,ds.Action_time AS downtime 
                            FROM dbmachinestatus ds JOIN databasemaster dm on ds.dbserver_id = dm.database_id
                            WHERE ds.action_type = 0 AND DATE(ds.Row_updated_date) = ?`, [formattedDate]);
    } else if (graph.fetchtype === 'weekly') {
      startDate = new Date(currentdate);
      startDate.setDate(currentdate.getDate() - 6);
      startDateFormatted = `${startDate.getFullYear()}-${String(startDate.getMonth() + 1).padStart(2, '0')}-${String(startDate.getDate()).padStart(2, '0')}`;
      sql = await db.query(`SELECT ds.dbserver_id,dm.databaseservername ,ds.Action_time AS downtime 
                            FROM dbmachinestatus ds JOIN databasemaster dm on ds.dbserver_id = dm.database_id
                            WHERE ds.action_type = 0 AND DATE(ds.Row_updated_date) BETWEEN ? AND ?`, [startDateFormatted, formattedDate]);
    } else if (graph.fetchtype === 'monthly') {
      startDate = new Date(currentdate);
      startDate.setDate(currentdate.getDate() - 29);
      startDateFormatted = `${startDate.getFullYear()}-${String(startDate.getMonth() + 1).padStart(2, '0')}-${String(startDate.getDate()).padStart(2, '0')}`;
      ...
      return helper.getErrorResponse(false, "Invalid fetching type. Please provide a valid fetching type (daily, weekly, monthly).", "DATABASE OVERALL STATUS");
    }
    ...
    sql.forEach(row => {
      ...
      if (dayFormatted) {
        databases[id].downtimes[dayFormatted] = { hour: hours, originalMin: minutes, min: dividedMinutes };
      }
    });
...
async function LoadbalanceDowntime(graph){
  try {
    if (!graph.hasOwnProperty('fetchtype')) {
      return helper.getErrorResponse(false, "Fetching type missing. Please provide the Fetching type", "FETCH LOAD BALANCER SERVER DOWNTIME", "");
    }
    ...
    if (graph.fetchtype === 'daily') {
      sql = await db.query(`SELECT ls.lbserver_id,lb.loadbalancer_name, ls.Action_time AS downtime 
                            FROM loadbalancerstatus ls JOIN loadbalancermaster lb ON lb.loadbalancer_id = ls.lbserver_id
                            WHERE ls.action_type = 0 AND DATE(ls.Row_updated_date) = ?`, [formattedDate]);
    } else if (graph.fetchtype === 'weekly') {
      startDate = new Date(currentdate);
      startDate.setDate(currentdate.getDate() - 6);
      startDateFormatted = `${startDate.getFullYear()}-${String(startDate.getMonth() + 1).padStart(2, '0')}-${String(startDate.getDate()).padStart(2, '0')}`;
      sql = await db.query(`SELECT ls.lbserver_id,lb.loadbalancer_name, ls.Action_time AS downtime 
      FROM loadbalancerstatus ls JOIN loadbalancermaster lb ON lb.loadbalancer_id = ls.lbserver_id
      WHERE ls.action_type = 0 AND DATE(ls.Row_updated_date) BETWEEN ? AND ?`, [startDateFormatted, formattedDate]);
    } else if (graph.fetchtype === 'monthly') {
      startDate = new Date(currentdate);
      startDate.setDate(currentdate.getDate() - 29);
      startDateFormatted = `${startDate.getFullYear()}-${String(startDate.getMonth() + 1).padStart(2, '0')}-${String(startDate.getDate()).padStart(2, '0')}`;
      ...
      return helper.getErrorResponse(false, "Invalid fetching type. Please provide a valid fetching type (daily, weekly, monthly).", "LOAD BALANCER SERVER OVERALL STATUS");
    }
    ...
    sql.forEach(row => {
      ...
      if (dayFormatted) {
        Loadbalancers[id].downtimes[dayFormatted] = { hour: hours, originalMin: minutes, min: dividedMinutes };
      }
    });
...
async function ComServerDowntime(graph){
  try {
    if (!graph.hasOwnProperty('fetchtype')) {
      return helper.getErrorResponse(false, "Fetching type missing. Please provide the Fetching type", "FETCH COMMUNICATION SERVER DOWNTIME", "");
    }
    ...
    if (graph.fetchtype === 'daily') {
      sql = await db.query(`SELECT cs.comserver_id,cm.Server_name,cs.Action_time AS downtime 
                            FROM comserverstatus cs JOIN communicationserver cm ON cs.comserver_id = cm.comserver_id
                            WHERE cs.action_type = 0 AND DATE(cs.Row_updated_date) = ?`, [formattedDate]);
    } else if (graph.fetchtype === 'weekly') {
      startDate = new Date(currentdate);
      startDate.setDate(currentdate.getDate() - 6);
      startDateFormatted = `${startDate.getFullYear()}-${String(startDate.getMonth() + 1).padStart(2, '0')}-${String(startDate.getDate()).padStart(2, '0')}`;
      sql = await db.query(`SELECT cs.comserver_id,cm.Server_name,cs.Action_time AS downtime 
      FROM comserverstatus cs JOIN communicationserver cm ON cs.comserver_id = cm.comserver_id
      WHERE cs.action_type = 0 AND DATE(cs.Row_updated_date)  BETWEEN ? AND ?`, [startDateFormatted, formattedDate]);
    } else if (graph.fetchtype === 'monthly') {
      startDate = new Date(currentdate);
      startDate.setDate(currentdate.getDate() - 29);
      startDateFormatted = `${startDate.getFullYear()}-${String(startDate.getMonth() + 1).padStart(2, '0')}-${String(startDate.getDate()).padStart(2, '0')}`;
      ...
      return helper.getErrorResponse(false, "Invalid fetching type. Please provide a valid fetching type (daily, weekly, monthly).", "COMMUNICATION SERVER OVERALL STATUS");
    }
...
async function WebServerDowntime(graph){
  try {
    if (!graph.hasOwnProperty('fetchtype')) {
      return helper.getErrorResponse(false, "Fetching type missing. Please provide the Fetching type", "FETCH WEB SERVER DOWNTIME", "");
    }
    ...
    if (graph.fetchtype === 'daily') {
      sql = await db.query(`SELECT ws.webserver_id,wm.webserver_name, ws.Action_time AS downtime 
                            FROM webserverstatus ws JOIN webservermaster wm ON ws.webserver_id = wm.webserver_id
                            WHERE ws.action_type = 0 AND DATE(ws.Row_updated_date) = ?`, [formattedDate]);
    } else if (graph.fetchtype === 'weekly') {
      startDate = new Date(currentdate);
      startDate.setDate(currentdate.getDate() - 6);
      startDateFormatted = `${startDate.getFullYear()}-${String(startDate.getMonth() + 1).padStart(2, '0')}-${String(startDate.getDate()).padStart(2, '0')}`;
      sql = await db.query(`SELECT ws.webserver_id,wm.webserver_name, ws.Action_time AS downtime 
      FROM webserverstatus ws JOIN webservermaster wm ON ws.webserver_id = wm.webserver_id
      WHERE ws.action_type = 0 AND DATE(ws.Row_updated_date) BETWEEN ? AND ?`, [startDateFormatted, formattedDate]);
    } else if (graph.fetchtype === 'monthly') {
      startDate = new Date(currentdate);
      startDate.setDate(currentdate.getDate() - 29);
      startDateFormatted = `${startDate.getFullYear()}-${String(startDate.getMonth() + 1).padStart(2, '0')}-${String(startDate.getDate()).padStart(2, '0')}`;
      ...
      return helper.getErrorResponse(false, "Invalid fetching type. Please provide a valid fetching type (daily, weekly, monthly).", "WEB SERVER OVERALL STATUS");
    }
    ...
    sql.forEach(row => {
      ...
      if (dayFormatted) {
        Webservers[id].downtimes[dayFormatted] = { hour: hours, originalMin: minutes, min: dividedMinutes };
      }
    });
...
async function FileServerDowntime(graph){
  try {
    if (!graph.hasOwnProperty('fetchtype')) {
      return helper.getErrorResponse(false, "Fetching type missing. Please provide the Fetching type", "FETCH FILE SERVER DOWNTIME", "");
    }
    const currentdate = new Date();
    const year = currentdate.getFullYear();
    const month = String(currentdate.getMonth() + 1).padStart(2, '0');
    ...
    if (graph.fetchtype === 'daily') {
      sql = await db.query(`SELECT fs.fileserver_id,fm.storageservername,fs.Action_time AS downtime 
                            FROM fileserverstatus fs JOIN storagepathmaster fm ON fm.storagepath_id = fs.fileserver_id
                            WHERE fs.action_type = 0 AND DATE(fs.Row_updated_date) = ?`, [formattedDate]);
    } else if (graph.fetchtype === 'weekly') {
      startDate = new Date(currentdate);
      startDate.setDate(currentdate.getDate() - 6);
      startDateFormatted = `${startDate.getFullYear()}-${String(startDate.getMonth() + 1).padStart(2, '0')}-${String(startDate.getDate()).padStart(2, '0')}`;
      sql = await db.query(`SELECT fs.fileserver_id,fm.storageservername,fs.Action_time AS downtime 
      FROM fileserverstatus fs JOIN storagepathmaster fm ON fm.storagepath_id = fs.fileserver_id
      WHERE fs.action_type = 0 AND DATE(fs.Row_updated_date) BETWEEN ? AND ?`, [startDateFormatted, formattedDate]);
    } else if (graph.fetchtype === 'monthly') {
      startDate = new Date(currentdate);
      startDate.setDate(currentdate.getDate() - 29);
      startDateFormatted = `${startDate.getFullYear()}-${String(startDate.getMonth() + 1).padStart(2, '0')}-${String(startDate.getDate()).padStart(2, '0')}`;
      ...
      return helper.getErrorResponse(false, "Invalid fetching type. Please provide a valid fetching type (daily, weekly, monthly).", "FILE SERVER OVERALL STATUS");
    }
    ...
    sql.forEach(row => {
      ...
      if (dayFormatted) {
        Fileservers[id].downtimes[dayFormatted] = { hour: hours, originalMin: minutes, min: dividedMinutes };
      }
    });
...
async function AppServerDowntime(graph){
  try {
    if (!graph.hasOwnProperty('fetchtype')) {
      return helper.getErrorResponse(false, "Fetching type missing. Please provide the Fetching type", "FETCH APPLICATION SERVER DOWNTIME", "");
    }
    const currentdate = new Date();
    const year = currentdate.getFullYear();
    const month = String(currentdate.getMonth() + 1).padStart(2, '0');
    ...
    if (graph.fetchtype === 'daily') {
      sql = await db.query(`SELECT ass.appserver_id, am.applicationserver_name, ass.Action_time AS downtime 
      FROM appserverstatus ass 
      JOIN applicationservermaster am ON ass.appserver_id = am.applicationserver_id
      WHERE ass.action_type = 0 AND DATE(ass.Row_updated_date) = ?`, [formattedDate]);
    } else if (graph.fetchtype === 'weekly') {
      startDate = new Date(currentdate);
      startDate.setDate(currentdate.getDate() - 6);
      startDateFormatted = `${startDate.getFullYear()}-${String(startDate.getMonth() + 1).padStart(2, '0')}-${String(startDate.getDate()).padStart(2, '0')}`;
      ...
      FROM appserverstatus ass 
      JOIN applicationservermaster am ON ass.appserver_id = am.applicationserver_id
      WHERE ass.action_type = 0 AND DATE(ass.Row_updated_date) BETWEEN ? AND ?`, [startDateFormatted, formattedDate]);
    } else if (graph.fetchtype === 'monthly') {
      startDate = new Date(currentdate);
      startDate.setDate(currentdate.getDate() - 29);
      startDateFormatted = `${startDate.getFullYear()}-${String(startDate.getMonth() + 1).padStart(2, '0')}-${String(startDate.getDate()).padStart(2, '0')}`;
      ...
      return helper.getErrorResponse(false, "Invalid fetching type. Please provide a valid fetching type (daily, weekly, monthly).", "APPLICATION SERVER OVERALL STATUS");
    }
...
async function DeviceBargraph(graph) {
  try {
    if(graph.hasOwnProperty('siteid') == false){
      return helper.getErrorResponse(false,"Site id missing. Please provide the site id.","GET DEVICE BAR GRAPH","");
    }
    // Fetching data for 1-hour intervals
    const motionDetectionCounts = await db.query(`
    SELECT
...
async function DashBoardPercentage(graph){
  try {
    ...
    const result = await db.query(`SELECT SUM(onlineservers) as onlineCount, SUM(offlineservers) as offlineCount FROM graphmaster`);
...
async function DashBoardBarGraph(graph){
  try {
    ...
    const result = await db.query(`SELECT graph_id,server_name,totalservers, onlineservers, offlineservers FROM graphmaster`);
...
async function DashBoardOverall(graph) {
  try {
    ...
    const result = await db.query(`
      ...
      FROM graphmaster 
      WHERE status = 1 AND DATE(laststoptime) = ?`, [formattedDate]);

    if (result.length > 0) {
      const downtimeReport = result.map((row, index) => {
        const lastStopTime = row.laststoptime ? new Date(row.laststoptime) : null;
        const dayFormatted = lastStopTime
          ? `${String(lastStopTime.getDate()).padStart(2, '0')}/${String(lastStopTime.getMonth() + 1).padStart(2, '0')}`
          : null;
