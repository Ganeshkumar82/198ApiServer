...
async function getEventAction(page, event){
   try {
    ...
    dt.Dept_Location, dc.Name1, dc.Contact_mobile1, dc.Contact_Email1,bm.Branch_id,bm.Branch_name,bm.site_starttime,bm.contact_person, cm.Camera_Status, es.eventpath,CASE WHEN EXISTS (SELECT 1 FROM ecsmaster ec WHERE ec.site_id = bm.branch_id and status = 1 and ECStype = 'Two way') THEN TRUE
    ...
    ELSE FALSE END AS oneway_status FROM eventmaster em LEFT JOIN 
...
async function Eventlistfilter(page, event) {
  try {
    ...
    if (querydata.feedback) {
      clSQL += ` AND em.Event_ID IN (SELECT Event_ID FROM ${logTable} WHERE feedback LIKE ?)`
      sqlParams.push(`%${querydata.feedback}%`);
    }

    if (querydata.eventvalidation) {
      if (querydata.eventvalidation === 'whatsapp') {
        clSQL += ` AND em.Event_ID IN (SELECT Event_ID FROM ${whatsappTable} )`;
      }
    }
  ...
  for (const img of imagePaths) {
    if (!imagePathsMap.has(img.event_id)) {
      imagePathsMap.set(img.event_id, []);
    }
    imagePathsMap.get(img.event_id).push(img.imagepath);
  }
  
  // Merge the image paths with each event row
  const resultData = rows.map(row => ({
    ...row,
    imagepaths: imagePathsMap.get(row.Event_ID) || []
  }));
...
async function getRecEvent(page = 1,event){
  ...
  // CHECK IF THE GIVEN SESSIONTOKEN IS VALID OR NOT
  const [result] =await db.spcall('CALL SP_STOKEN_CHECK(?,@result,@custid,@custname,@custemail); select @result,@custid,@custname,@custemail',[event.STOKEN]);
  // console.log(`spcallllllllllllll ${JSON.stringify([result])}`)
  const objectvalue = result[1][0];
  const userid = objectvalue["@result"];
  // console.log("event list userid ->"+ userid);
  if(userid == null){
...
async function GetUserReport(page,event){
  ...
  if(querydata.filtertype == 'all'){
    ...
    ELSE FALSE END AS twoway_status,CASE WHEN EXISTS (SELECT 1 FROM ecsmaster ec WHERE ec.site_id = bm.branch_id and status = 1 and ECStype = 'PA system') THEN TRUE
    ELSE FALSE END AS oneway_status FROM eventmaster em JOIN cameramaster cm ON cm.camera_id = em.analyticsource_id JOIN devicemaster dm ON dm.device_id = cm.device_id
    ...
    sql = `SELECT em.Event_ID, em.Event_Name, bm.whatsappgroupname, em.Row_updated_date,DATE_FORMAT(em.enddate, '%Y-%m-%d %H:%i:%s') AS eventtime, em.Alertmessage, em.IsHumanDetected, 
    cm.camera_id, cm.camera_name, dm.device_name, dm.IP_Domain, dm.RTSP_port, dm.IP_port, dm.IP_Uname, dm.IP_Pwd, dm.short_name, dm.SDK_ID, dm.device_id, dt.Dept_name, dt.Dept_Location, 
    dc.Name1, dc.Contact_mobile1, dc.Contact_Email1, bm.Branch_name,bm.site_starttime Notifytime,bm.contact_person, cm.Camera_Status, ep.imagepath,CASE WHEN EXISTS (SELECT 1 FROM ecsmaster ec WHERE ec.site_id = bm.branch_id and status = 1 and ECStype = 'Two way') THEN TRUE
    ELSE FALSE END AS twoway_status,CASE WHEN EXISTS (SELECT 1 FROM ecsmaster ec WHERE ec.site_id = bm.branch_id and status = 1 and ECStype = 'PA system') THEN TRUE
    ELSE FALSE END AS oneway_status FROM eventmaster em JOIN cameramaster cm ON cm.camera_id = em.analyticsource_id JOIN devicemaster dm ON dm.device_id = cm.device_id JOIN deptmaster dt ON dt.dept_id = dm.dept_id 
    LEFT JOIN deptcontacts dc ON dc.dept_id = dt.dept_id JOIN branchmaster bm ON bm.branch_id = dt.branch_id LEFT JOIN (SELECT Event_ID, MIN(detected_file) AS imagepath FROM eventaistatus GROUP BY Event_ID
    ...
    sql = `SELECT em.Event_ID, em.Event_Name, bm.whatsappgroupname, em.Row_updated_date,DATE_FORMAT(em.enddate, '%Y-%m-%d %H:%i:%s') AS eventtime, em.Alertmessage,em.IsHumanDetected,
    cm.camera_id, cm.camera_name, dm.device_name, dm.IP_Domain,dm.RTSP_port, dm.IP_port, dm.IP_Uname, dm.IP_Pwd, dm.short_name, dm.SDK_ID, dm.device_id, dt.Dept_name,
    dt.Dept_Location, dc.Name1, dc.Contact_mobile1,dc.Contact_Email1, bm.Branch_name, bm.site_starttime Notifytime, bm.contact_person,cm.Camera_Status, ep.imagepath, el.feedback,CASE WHEN EXISTS (SELECT 1 FROM ecsmaster ec WHERE ec.site_id = bm.branch_id and status = 1 and ECStype = 'Two way') THEN TRUE
    ELSE FALSE END AS twoway_status,CASE WHEN EXISTS (SELECT 1 FROM ecsmaster ec WHERE ec.site_id = bm.branch_id and status = 1 and ECStype = 'PA system') THEN TRUE
...
async function Acknowledge(page,event){
  ...
  SELECT em.Event_ID, em.Event_Name, bm.whatsappgroupname, em.Row_updated_date,
         ...
         bm.contact_person, cm.Camera_Status, ep.imagepath, el.feedback, el.Row_upd_date,CASE WHEN EXISTS (SELECT 1 FROM ecsmaster ec WHERE ec.site_id = bm.branch_id and status = 1 and ECStype = 'Two way') THEN TRUE
         ELSE FALSE END AS twoway_status,CASE WHEN EXISTS (SELECT 1 FROM ecsmaster ec WHERE ec.site_id = bm.branch_id and status = 1 and ECStype = 'PA system') THEN TRUE
         ELSE FALSE END AS oneway_status
  FROM eventmaster em
  JOIN cameramaster cm ON cm.camera_id = em.analyticsource_id
