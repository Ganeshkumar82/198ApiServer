...
async function getEventAction(page, event){
   try {
    ...
    dt.Dept_Location, dc.Name1, dc.Contact_mobile1, dc.Contact_Email1,bm.Branch_id,bm.Branch_name,bm.site_starttime,bm.contact_person, cm.Camera_Status, es.eventpath FROM eventmaster em LEFT JOIN 
    eventstatus es ON es.Event_id = em.Event_ID JOIN cameramaster cm ON cm.camera_id = em.analyticsource_id JOIN devicemaster dm ON dm.device_id = cm.device_id JOIN 
...
async function Eventlistfilter(page, event) {
  try {
    ...
    // const eventLogTable = `eventlog${tableSuffix}`;
    // const whatsappLogTable = `whatsapplog${tableSuffix}`;
    // const eventAiStatusTable = `eventaistatus${tableSuffix}`;
    // const eventMasterTable = `eventmaster${tableSuffix}`
    ...
    clSQL += ` GROUP BY em.Event_ID ORDER BY em.Row_updated_date LIMIT ${offset},${config.listPerPage}`;
    ...
    try{
      ...
      const imagePaths = await db.query1(`SELECT event_id,LOCATE('Volumes', detected_file),detected_file as imagepath FROM eventaistatus WHERE event_id IN (${eventIds});`);
    
  // Flattening the array of event IDs, if needed

  // const imagePaths = await db.query1(query, [isAfter7AM, eventIds]);
     
    const imagePathsArray = imagePaths.rows || imagePaths;
    // Merge the results from the two queries
    var resultData;
    if(imagePathsArray){
      resultData = rows.map(row => {
      const imagePathRecord = imagePathsArray.find(image => image.event_id === row.Event_ID);
      row.imagepath = imagePathRecord ? imagePathRecord.imagepath : null;
      return row;
    });
  }else{
    return '';
...
//     const clSQL = `
//       SELECT em.Event_ID, em.Event_Name, em.Row_updated_date, 
//              DATE_FORMAT(em.enddate, '%Y-%m-%d %H:%i:%s') AS eventtime, 
//              em.Alertmessage, em.IsHumanDetected, 
//              es.imagepath, el.feedback,
//              CASE WHEN wl.Event_ID IS NOT NULL THEN 'Acknowledged' ELSE 'Unacknowledged' END AS eventstatus
//       FROM (
//         SELECT em.Event_ID, em.Event_Name, em.Row_updated_date, em.enddate, em.Alertmessage, em.IsHumanDetected
//         FROM eventmaster_20241014 em
//         WHERE em.enddate BETWEEN ? AND ?
        
//         UNION ALL
        
//         SELECT em.Event_ID, em.Event_Name, em.Row_updated_date, em.enddate, em.Alertmessage, em.IsHumanDetected
//         FROM eventmaster_20241015 em
//         WHERE em.enddate BETWEEN ? AND ?
...
//       ) em
//       LEFT JOIN (
//         SELECT es.event_id, MIN(es.detected_file) AS imagepath
//         FROM (
//           SELECT event_id, detected_file FROM eventaistatus_20241014
//           UNION ALL
//           SELECT event_id, detected_file FROM eventaistatus_20241015
//           UNION ALL
//           SELECT event_id, detected_file FROM eventaistatus
//         ) es
//         GROUP BY es.event_id
//       ) es ON em.Event_ID = es.event_id
//       LEFT JOIN (
//         SELECT el.event_id, el.feedback
//         FROM (
//           SELECT event_id, feedback FROM eventlog_20241014
//           UNION ALL
//           SELECT event_id, feedback FROM eventlog_20241015
//           UNION ALL
//           SELECT event_id, feedback FROM eventlog
//         ) el
//       ) el ON em.Event_ID = el.event_id
//       LEFT JOIN (
//         SELECT wl.Event_ID
//         FROM (
//           SELECT Event_ID FROM whatsapplog_20241014 WHERE Row_updated_date BETWEEN ? AND ?
//           UNION ALL
//           SELECT Event_ID FROM whatsapplog_20241015 WHERE Row_updated_date BETWEEN ? AND ?
//           UNION ALL
//           SELECT Event_ID FROM whatsapplog WHERE Row_updated_date BETWEEN ? AND ?
//         ) wl
//       ) wl ON em.Event_ID = wl.Event_ID
//       ORDER BY em.Row_updated_date
//       LIMIT ?, ?;
//     `;

//     // Add pagination params
//     sqlParams.push(offset, config.eventlistpage);
...
async function getUnAcknoEvent(page, event) {
  try {
    ...
    dc.Name1, dc.Contact_mobile1, dc.Contact_Email1,bm.Branch_id,bm.Branch_name,bm.site_starttime,bm.contact_person, cm.Camera_Status, es.eventpath FROM eventmaster em JOIN eventstatus es ON es.Event_id = em.Event_ID 
    ...
    WHERE el.Event_ID IS NULL AND wl.Event_id IS NULL AND em.Event_Name NOT LIKE 'Tampering%' AND em.Event_Name NOT LIKE 'HD D%' AND em.Event_Name NOT LIKE 'Video%' 
...
-- Replacing NOT IN with LEFT JOIN and IS NULL for better performance
LEFT JOIN 
...
if(querydata.hasOwnProperty('eventname')==false){
  ...
  let sql =''
  sql = `select dm.IP_Domain,dm.IP_Port,dm.IP_Uname,dm.IP_pwd,dm.SDK_ID,dm.device_name,dm.Device_id,cm.Camera_status,cm.camera_name,bm.Branch_name,bm.branch_id,bm.site_starttime,bm.whatsappgroupname,dt.Dept_location from cameramaster cm ,devicemaster dm,branchmaster bm,deptmaster dt where bm.Branch_id = dt.Branch_id and
  ...
  const siteid = rows[0].branch_id
  var camera_no = rows[0].camera_name;
  const Notifytime = rows[0].site_starttime;
  ...
  try
    {
      const returnstr = JSON.stringify({code:true,message:'Event created Successfully',cameraid,eventid,eventname,sitename,ipaddress,port,username,password,SDK_ID,deviceid,devicename,channelno,camerastatus,whatsappgroupname,Location,Notifytime,siteid});
      if (secret!="")
      {
      ...
      {
        return ({code:true,message:'Event created Successfully',cameraid,eventid,eventname,sitename,ipaddress,port,username,password,SDK_ID,deviceid,devicename,channelno,camerastatus,whatsappgroupname,Location,Notifytime,siteid});
      }
    }
    catch(Ex)
    {
...
async function GetUserReport(page,event){
  ...
  if(querydata.filtertype == 'all'){
    ...
    cm.camera_id,cm.camera_name,dm.device_name,dm.IP_Domain,dm.RTSP_port, dm.IP_port, dm.IP_Uname, dm.IP_Pwd, dm.short_name, dm.SDK_ID,  dm.device_id, dt.Dept_name,dt.Dept_Location, 
    dc.Name1,dc.Contact_mobile1,dc.Contact_Email1,bm.Branch_name,bm.site_starttime Notifytime,bm.contact_person, cm.Camera_Status, MIN(es.detected_file) AS imagepath,CASE WHEN el.Event_ID IS NOT NULL OR wl.Event_ID IS NOT NULL THEN 'Acknowledged' 
    ...
    sql = `SELECT em.Event_ID, em.Event_Name, bm.whatsappgroupname, em.Row_updated_date,DATE_FORMAT(em.enddate, '%Y-%m-%d %H:%i:%s') AS eventtime, em.Alertmessage, em.IsHumanDetected, 
    cm.camera_id, cm.camera_name, dm.device_name, dm.IP_Domain, dm.RTSP_port, dm.IP_port, dm.IP_Uname, dm.IP_Pwd, dm.short_name, dm.SDK_ID, dm.device_id, dt.Dept_name, dt.Dept_Location, 
    dc.Name1, dc.Contact_mobile1, dc.Contact_Email1, bm.Branch_name,bm.site_starttime Notifytime,bm.contact_person, cm.Camera_Status, ep.imagepath FROM eventmaster em JOIN cameramaster cm ON cm.camera_id = em.analyticsource_id JOIN devicemaster dm ON dm.device_id = cm.device_id JOIN deptmaster dt ON dt.dept_id = dm.dept_id 
    LEFT JOIN deptcontacts dc ON dc.dept_id = dt.dept_id JOIN branchmaster bm ON bm.branch_id = dt.branch_id LEFT JOIN (SELECT Event_ID, MIN(detected_file) AS imagepath FROM eventaistatus GROUP BY Event_ID
    ...
    sql = `SELECT em.Event_ID, em.Event_Name, bm.whatsappgroupname, em.Row_updated_date,DATE_FORMAT(em.enddate, '%Y-%m-%d %H:%i:%s') AS eventtime, em.Alertmessage,em.IsHumanDetected,
    cm.camera_id, cm.camera_name, dm.device_name, dm.IP_Domain,dm.RTSP_port, dm.IP_port, dm.IP_Uname, dm.IP_Pwd, dm.short_name, dm.SDK_ID, dm.device_id, dt.Dept_name,
    dt.Dept_Location, dc.Name1, dc.Contact_mobile1,dc.Contact_Email1, bm.Branch_name, bm.site_starttime Notifytime, bm.contact_person,cm.Camera_Status, ep.imagepath, el.feedback
    FROM eventmaster em JOIN cameramaster cm ON cm.camera_id = em.analyticsource_id JOIN devicemaster dm ON dm.device_id = cm.device_id JOIN deptmaster dt ON dt.dept_id = dm.dept_id
    LEFT JOIN deptcontacts dc ON dc.dept_id = dt.dept_id JOIN branchmaster bm ON bm.branch_id = dt.branch_id
    LEFT JOIN (SELECT Event_ID, MIN(eventpath) AS imagepath FROM eventstatus GROUP BY Event_ID) ep ON ep.Event_ID = em.Event_ID
    JOIN eventlog el ON el.event_id = em.Event_ID WHERE EXISTS (SELECT 1 FROM eventuser eu WHERE eu.Event_id = em.Event_ID AND eu.user_id = ${querydata.userid}) and 
...
async function Acknowledge(page,event){
  ...
  SELECT em.Event_ID, em.Event_Name, bm.whatsappgroupname, em.Row_updated_date,
         ...
         cm.camera_id, cm.camera_name, dm.device_name, dm.IP_Domain, dm.RTSP_port, dm.IP_port, dm.IP_Uname, 
         dm.IP_Pwd, dm.short_name, dm.SDK_ID, dm.device_id, dt.Dept_name, dt.Dept_Location,
         dc.Name1, dc.Contact_mobile1, dc.Contact_Email1, bm.Branch_name, bm.site_starttime AS Notifytime, 
         bm.contact_person, cm.Camera_Status, ep.imagepath, el.feedback, el.Row_upd_date
  FROM eventmaster em
  JOIN cameramaster cm ON cm.camera_id = em.analyticsource_id
  JOIN devicemaster dm ON dm.device_id = cm.device_id
  JOIN deptmaster dt ON dt.dept_id = dm.dept_id
