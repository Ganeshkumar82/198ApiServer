...
var utype  = 3;
  ...
  if (objectValue["@result"]==null) {    
      return helper.getErrorResponse(false,"Incorrect username or password. Please verify your username/password and try again.","LOGIN",user.Secret);
  }
  else
  {
    console.log("Login, result->"+objectValue["@result"]);
    const [result2] = await db.spcall('CALL SP_USER_LOGIN(?,?,@result,@breakstatus);select @result,@breakstatus;',[objectValue["@result"],clientIp]);
    const objectValue1 = result2[1][0];
    const SESSIONTOKEN = objectValue1["@result"];
    const breakstatus = objectValue1["@breakstatus"];
    // console.log("Login,SESSIONTOKEN->"+objectValue1["@result"]);
    try
    {
      const returnstr = JSON.stringify({code:true,message:'Login successfull',userid,username,customerid,userdesign,useraccess,SESSIONTOKEN,breakstatus});
      if (user.Secret!="")
      {
      ...
      {
        return ({code:true,message:'Login successfull',userid,username,customerid,userdesign,useraccess,SESSIONTOKEN,breakstatus});
      }
    }
    catch(Ex)
    {
      return ({code:true,message:'Login successfull',userid,username,userdesign,useraccess,SESSIONTOKEN,breakstatus});
    }
...
async function Logout(user){
  if(user.hasOwnProperty('STOKEN')==false){
    return helper.getErrorResponse(false,"Login sessiontoken missing. Please provide the Login sessiontoken","LOGOUT","");
  }
  //CHECK IF THE SESSIONTOKEN SIZE IS VALID OR NOT
  if(user.STOKEN.length > 50 || user.STOKEN.length  < 30){
    return helper.getErrorResponse(false,"Login sessiontoken size invalid. Please provide the valid Sessiontoken","LOGOUT","");
  }
  ...
  if(userid == null){
    return helper.getErrorResponse(false,"Login sessiontoken Invalid. Please provide the valid sessiontoken","LOGOUT","");
  }
  ...
  message = data['@result'];
  return helper.getSuccessResponse(true,message,message,secret);
  }catch(er){
     return helper.getErrorResponse(false,'Internal server error. Please contact Administrator',er.message,secret);
  }
}
...
async function AddUser(user){
  try {
    ...
    if(!querydata.hasOwnProperty('aadharnumber') || querydata.aadharnumber == ''){
      return helper.getErrorResponse(false,"User aadhar missing. Please provide the Aadhar number","Please enter the Aadhar number",secret);
    }
    if(!querydata.hasOwnProperty('pannumber') || querydata.pannumber == ''){
      return helper.getErrorResponse(false,"Pan number missing. Please provide the pan number","Please enter the Pan number",secret);
    }
    if(!querydata.hasOwnProperty('accountnumber') || querydata.accountnumber == ''){
      return helper.getErrorResponse(false,"Account number missing. Please provide the account number","Please enter the Account number",secret);
    }
    if(!querydata.hasOwnProperty('permanentaddress') || querydata.permanentaddress == ''){
      return helper.getErrorResponse(false,"Permanent address missing. Please provide the Permanent address","Please enter the permanent address",secret);
    }
    if(!querydata.hasOwnProperty("temporaryaddress") || querydata.temporaryaddress == ''){
      return helper.getErrorResponse(false,"Temporary address missing. Please provide the temporary address.","Please enter the temporary address",secret);
    }
    if(!querydata.hasOwnProperty("fullname") || querydata.fullname == ''){
      return helper.getErrorResponse(false,'Full name missing. Please provide the full name',"Please enter the full name",secret);
    }
    ...
    if (phonePattern1.test(querydata.phonenumber) || phonePattern2.test(querydata.phonenumber)) {
       ...
       const [sql] = await db.spcall(`CALL SP_CUST_USER_ADD(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,@userid); select @userid;`,[0,querydata.username,querydata.emailid,querydata.username,querydata.password,0,querydata.userdesign,querydata.phonenumber,querydata.userid,querydata.aadharnumber,querydata.pannumber,querydata.accountnumber,querydata.permanentaddress,querydata.temporaryaddress,querydata.fullname]);
...
async function UserBreak(user){
  try {
    // Check if the session token exists
    if (!user.hasOwnProperty('STOKEN')) {
      return helper.getErrorResponse(false, "Login session token missing. Please provide the Login session token", "UPDATE USER BREAK FINISH", "");
    }
    
    // Validate session token length
    if (user.STOKEN.length > 50 || user.STOKEN.length < 30) {
      return helper.getErrorResponse(false, "Login session token size invalid. Please provide the valid Session token", "UPDATE USER BREAK FINISH", "");
    }
    ...
    if (userid == null) {
      return helper.getErrorResponse(false, "Login session token Invalid. Please provide the valid session token", "UPDATE USER BREAK FINISH", "");
    }
    
    // Check if querystring is provided
    if (!user.hasOwnProperty("querystring")) {
      return helper.getErrorResponse(false, "Querystring missing. Please provide the querystring","UPDATE USER BREAK FINISH", "");
    }
    ...
    try {
      ...
      return helper.getErrorResponse(false, "Querystring Invalid error. Please provide the valid querystring.","UPDATE USER BREAK FINISH", secret);
    }
    ...
    try {
      ...
      return helper.getErrorResponse(false, "Querystring JSON error. Please provide valid JSON", "UPDATE USER BREAK FINISH", secret);
    }
    if (!querydata.hasOwnProperty('userid') || querydata.userid == '') {
      return helper.getErrorResponse(false, "User id missing. Please provide the User id", "Please enter the Userid", secret);
    }

    const sql = await db.query(`Update usersbreak set Break_finished_time = NOW() ,status = 0 where user_id = ? and status =1`,[querydata.userid]);
    const sql1 = await db.query(`SELECT TIMEDIFF(Break_finished_time, Break_time) AS BreakDuration FROM usersbreak WHERE user_id = ? ORDER BY Break_finished_time DESC LIMIT 1;`,[querydata.userid]);
    if(sql.affectedRows && sql1[0]){
      return helper.getSuccessResponse(true,`success`,sql1[0].BreakDuration,secret);
    }else{
      return helper.getSuccessResponse(false,`failed`,"UPDATE USER BREAK COMPLETION",secret);
    }
  }catch(er){
    console.log(er);
    return helper.getErrorResponse(false,"Internal error. Please contact Administration",er,secret);
