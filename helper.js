...
{
  const [result1] = await db.spcall('CALL SP_API_CHECK(?,?,@isValid);select @isValid;',[apikey,apisecret]);
...
async function getCustomerID(srcID,srcType)
{
  ...
  var returnID = 0;
  if (btype==0)
    sSQLstr = "SELECT customerreg_id from customermaster where status=1 and customer_id="+srcID;
  else
    sSQLstr = "SELECT customerreg_id from customermaster where status=1 and customer_id in (select customer_id from branchmaster where branch_id="+srcID+")";
...
{
  const result = await db.query("select setting_value from serversettings where setting_name='"+SettingName+"';");    
...
function getOffset1(currentPage , listPerPage) {
  if(currentPage == 1){
...
console.log(formatDate(currentDate, 'yyyyMMdd')); // Output: 20250102
console.log(formatDate(currentDate, 'yyyy-MM-dd HH:mm:ss')); // Output: 2025-01-02 14:30:45
